...
public class OkCoinStreamingMarketDataService implements StreamingMarketDataService {
    ...
    /**
     * #### future ####
     * 1. ok_sub_futureusd_X_ticker_Y   订阅合约行情
     * 1. ok_sub_futureusd_X_ticker_Y   Subscribe Contract Market Price
     *
     * @param currencyPair Currency pair of the ticker
     * @param contract     {@link FuturesContract}
     * @return
     */
    public Observable<FutureTicker> getFutureTicker(CurrencyPair currencyPair, FuturesContract contract) {
        String channel = String.format("ok_sub_future%s_%s_ticker_%s", currencyPair.counter.toString().toLowerCase(), currencyPair.base.toString().toLowerCase(), contract.getName());
        return service.subscribeChannel(channel).map(s -> mapper.treeToValue(s.get("data"), FutureTicker.class));
    }
