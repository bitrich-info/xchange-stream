...
public class HuobiStreamingMarketDataService implements StreamingMarketDataService {
    ...
    public Observable<OrderBook> getOrderBook(CurrencyPair currencyPair, Object... args) {
      String channelName = "market." + HuobiUtils.createHuobiCurrencyPair(currencyPair) +
          ".depth." + (args.length == 0 ? "step0" : args[0].toString());
      ...
      return streamingService.subscribeChannel(channelName)
          .map(message -> {
            ...
            for (int i = 0; i < asks.size(); i++) {
              ...
              LimitOrder lo = new LimitOrder(Order.OrderType.ASK, amount, currencyPair, null, ts, price);
              askOrders.add(lo);
            }
            ...
            for (int i = 0; i < bids.size(); i++) {
              ...
              BigDecimal price = order.get(0).decimalValue();
              BigDecimal amount = order.get(1).decimalValue();
              LimitOrder lo = new LimitOrder(Order.OrderType.BID, amount, currencyPair,null, ts, price);
              bidOrders.add(lo);
            }
            return orderBook;
          });
    ...
    public Observable<Ticker> getTicker(CurrencyPair currencyPair, Object... args) {
      String channelName = "market." + HuobiUtils.createHuobiCurrencyPair(currencyPair) + ".kline." +
    ...
    public Observable<Trade> getTrades(CurrencyPair currencyPair, Object... args) {
      String channelName = "market." + HuobiUtils.createHuobiCurrencyPair(currencyPair) + ".trade.detail";
        return streamingService.subscribeChannel(channelName)
                .map(message -> {
