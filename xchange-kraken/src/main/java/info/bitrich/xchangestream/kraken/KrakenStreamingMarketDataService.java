...
public class KrakenStreamingMarketDataService implements StreamingMarketDataService {
    ...
    public Observable<Ticker> getTicker(CurrencyPair currencyPair, Object... args) {
        ...
        return subscribe(channelName, MIN_DATA_ARRAY_SIZE, null)
                .map(jsonParseResult -> {
                    ...
                    KrakenTicker krakenTicker = new KrakenTicker(
                                ...
                                tradeList -> {
                                    ...
                                    return KrakenAdapters.adaptTrade(
                                            new KrakenPublicTrade(
                                                    ...
                                                    orderType == null ? null : KrakenOrderType.fromString(orderType),
    ...
    public Observable<List> subscribe(String channelName, int maxItems, Integer depth) {
        return service.subscribeChannel(channelName, depth)
                .filter(JsonNode::isArray)
                .filter(Objects::nonNull)
                .map(jsonNode -> StreamingObjectMapperHelper.getObjectMapper().treeToValue(jsonNode, List.class))
                .filter(list -> {
                    if (list.size() < maxItems) {
                        LOG.error("Invalid message in channel {}. It contains {} array items but expected at least {}", channelName, list.size(), maxItems);
                        return false;
                    }
                    return true;
                });
    ...
    private String getChannelName(KrakenSubscriptionName subscriptionName, CurrencyPair currencyPair) {
    ...
    private int parseOrderBookSize(Object[] args) {
        if (args != null && args.length > 0) {
            ...
            if (Number.class.isAssignableFrom(obSizeParam.getClass())) {
                ...
                LOG.error("Invalid order book size {}. Valid values: {}. Default order book size has been used: {}",
                        obSize, ArrayUtils.toString(KRAKEN_VALID_ORDER_BOOK_SIZES), ORDER_BOOK_SIZE_DEFAULT);
                return ORDER_BOOK_SIZE_DEFAULT;
            }
            LOG.error("Order book size param type {} is invalid. Expected: {}. Default order book size has been used {}",
                    obSizeParam.getClass().getName(), Number.class.getName(), ORDER_BOOK_SIZE_DEFAULT);
            return ORDER_BOOK_SIZE_DEFAULT;
        }

        LOG.debug("Order book size param has not been specified. Default order book size has been used: {}", ORDER_BOOK_SIZE_DEFAULT);
        return ORDER_BOOK_SIZE_DEFAULT;
    }
